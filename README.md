...
4. **Backend service, PostgreSql database, CRUD**: Backend service provided by server.ts, routes/\*, and controllers/\*, models/\*. The models/database.sql describes the database structure and models/db.ts creates the connection the sql server. All routes are in routes/index.ts.
